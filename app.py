...
SIMILARITY_FILE_URL = os.getenv(
    "SIMILARITY_FILE_URL",
    "https://drive.google.com/file/d/1C1nxZKKYLgzt6vfl0D7LLMfGvJnl6mU4/view?usp=sharing",
)
SIMILARITY_FILE_PATH = BASE_DIR / "similarity.pkl"
MOVIES_DICT_FILE_PATH = BASE_DIR / "movies_dict.pkl"
...
def load_similarity_data():
    ...
    with SIMILARITY_FILE_PATH.open("rb") as similarity_file:
        return pkl.load(similarity_file)


@st.cache_data(show_spinner=False)
def load_movies_dataframe():
    with MOVIES_DICT_FILE_PATH.open("rb") as movies_file:
...
try:
    ...
    movies = load_movies_dataframe()
except Exception as error:
    st.error(f"Failed to load app data: {error}")
    st.stop()
